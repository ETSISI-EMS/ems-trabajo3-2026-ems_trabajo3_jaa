	...
	private void procesarLinea(String linea) throws EmsInvalidTypeException,EmsInvalidNumberOfDataException,EmsDuplicatePersonException,EmsDuplicateLocationException{
		...
		if (!datos[0].equals("PERSONA") && !datos[0].equals("LOCALIZACION")) {
			...
			if (datos.length != Constantes.MAX_DATOS_PERSONA) {
				throw new EmsInvalidNumberOfDataException("El número de datos para PERSONA es menor de "+Constantes.MAX_DATOS_PERSONA);
			}
			this.poblacion.addPersona(this.crearPersona(datos));
		}else if (datos[0].equals("LOCALIZACION")) {
			if (datos.length != Constantes.MAX_DATOS_LOCALIZACION) {
				throw new EmsInvalidNumberOfDataException(
						"El número de datos para LOCALIZACION es menor de "+Constantes.MAX_DATOS_LOCALIZACION);
			}
